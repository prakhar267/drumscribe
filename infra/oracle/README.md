...
sum of every running A1 instance inside that allowance. The DrumToScore
production VM now uses 2 OCPUs and 12 GB RAM, plus a 46.6 GB boot volume inside
the 200 GB Always Free block-volume allowance. Do not run another A1 instance
alongside production except for a short, verified recovery operation.
...
public API and scheduler on the two-OCPU beta host. Keep Celery concurrency at
...
on the former 1-OCPU/6-GB shape and peaked at 3.70 GiB memory. Treat that result
as the pre-resize baseline until the same sealed probe is repeated on the
current 2-OCPU/12-GB shape. Before any later resize, inspect all tenancy A1
allocations and confirm the new total remains inside the documented Always Free
allowance; never select a paid shape or attach a payment method without explicit
approval.
