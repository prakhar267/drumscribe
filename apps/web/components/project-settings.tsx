  ...
  return (
    <div className="settings-layout">
      ...
      <div className="settings-content">
        ...
        <section className="surface settings-section danger-zone" id="danger"><h2>Delete project</h2><p>Moves the project and private audio to deleted items. Permanent object removal runs after the configured recovery window.</p>{deleted ? <div className="notice">Project moved to deleted items. <button type="button" className="inline-action" onClick={() => { setDeleted(false); void api.restoreProject(projectId); }}>Undo</button></div> : <button className="button button-danger button-small" type="button" onClick={() => { setDeleted(true); void api.deleteProject(projectId).catch(() => setDeleted(false)); }}><Trash2 size={14} /> Delete project</button>}</section>
      </div>
    </div>
  );
}
