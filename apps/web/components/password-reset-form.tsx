...
export function PasswordResetForm() {
  ...
  return (
    <div className="auth-form">
      ...
      <form onSubmit={(event) => { event.preventDefault(); setBusy(true); setError(null); void getNeonAuthClient().then((authClient) => authClient.resetPassword({ newPassword: password, token })).then(({ error: resetError }) => { if (resetError) throw new Error(resetError.message || "Password reset failed."); setDone(true); }).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : "Password reset failed.")).finally(() => setBusy(false)); }}>
