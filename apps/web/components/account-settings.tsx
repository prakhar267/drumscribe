...
import { deleteAccountEverywhere, signOutEverywhere } from "@/lib/auth/client";
  ...
  return (
    <div className="settings-layout">
      ...
      <div className="settings-content">
        <section className="surface settings-section" id="profile"><h2>Profile</h2><p>Your authenticated account details. DrumToScore does not invent or publicly display a profile name.</p><div className="settings-form-grid"><label className="field"><span className="field-label">Account type</span><input className="text-input" value={accountKind} readOnly /></label><label className="field"><span className="field-label">Email</span><input className="text-input" value={email} readOnly /></label></div><div style={{ display: "flex", gap: 10, marginTop: 18 }}><Link className="button button-small" href="/auth">Add or change email</Link><button className="button button-small" type="button" onClick={() => { void signOutEverywhere().then(() => { router.replace("/"); router.refresh(); }); }}><LogOut size={14} /> Sign out</button></div></section>
