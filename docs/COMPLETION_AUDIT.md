...
- The company owner approved the exact pinned self-hosted `htdemucs_ft`, ADTOF and Beat This artifacts for commercial inference under `OWNER-ATTESTATION-2026-09-05`. The runtime now permits only those approved versions and records the approval reference in production provider metadata.
- A fresh 11-track real full-mixture rerun regenerated every Demucs stem and ADTOF prediction and measured 82.99% six-family micro F1 at 50 ms. This supplies a real quality baseline while keeping the broad 90% claim blocked.
...
The remaining launch gates are production cloud/domain/database/queue/storage/email/monitoring verification, customer-facing legal approval, staging security/restore/browser acceptance, and resolution of the GitHub Actions billing/spending-limit block. The selected self-hosted ML path is commercially approved, but its 82.99% broad real-mixture benchmark remains below the 90% accuracy target.
