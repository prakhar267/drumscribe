...
| Public API, worker, and scheduler | Oracle Cloud Always Free Ampere A1, Mumbai | Root-only environment file, Docker Compose, Caddy TLS | `drumscribe-production` is live on 4 OCPUs/24 GB with a 46.6 GB boot volume, consuming the console-confirmed free A1 allowance. API readiness, Celery worker, Celery Beat, model-bundle verification, Redis TLS, firewalling, and public HTTPS are verified. The 180-second capacity probe completed in 10m26s with zero restarts. No load balancer, NAT gateway, reserved IP, paid plan, or card action was used. |
