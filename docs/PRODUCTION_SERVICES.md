...
| Public API, worker, and scheduler | Oracle Cloud Always Free Ampere A1, Mumbai | Root-only environment file, Docker Compose, Caddy TLS | `drumscribe-production` is live on 4 OCPUs/24 GB with a 46.6 GB boot volume, consuming the console-confirmed free A1 allowance. API readiness, one two-process Celery worker service, Celery Beat, model-bundle verification, private Valkey, firewalling, and public HTTPS are verified. A sealed two-job production probe removed queue serialization without increasing either job's observed completion time; worker memory remained near 1.2 GiB. No load balancer, NAT gateway, reserved IP, paid plan, or card action was used. |
