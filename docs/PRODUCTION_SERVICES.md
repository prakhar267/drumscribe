...
| Public API, worker, and scheduler | Northflank Sandbox | Container environment variables and TLS hostname | API and production ML worker images, a verified private model bundle, migration command, hourly retention command, health paths, and scaling contract are prepared. Deployment is blocked only on the account owner's GitHub authorization in Northflank. Sandbox is suitable for pre-launch validation, not an SLA launch. Oracle is no longer in the selected topology. |
