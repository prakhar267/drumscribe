...
- [ ] Replace ambiguous provider strings with explicit categories: `PRODUCTION_COMMERCIAL`, `DEVELOPMENT_RESEARCH`, `TEST_FIXTURE`.
- [ ] Require provider identity, model/version, request ID, duration, confidence summary, normalized error, raw metadata, cost and retention metadata.
- [ ] Make production startup reject fixture, mock, passthrough, non-commercial and unapproved research providers without fallback.
- [ ] Keep fixture providers only for isolated tests and the explicitly labelled demo.
- [ ] Add anti-fake tests proving different recordings cannot receive the same duration-derived output in a production-capable configuration.

### P0.2 Real source separation

- [ ] Implement an HTTP commercial separation adapter with authenticated upload/reference input, status polling, timeout, retry/idempotency, output validation and private drum-stem ingestion.
- [ ] Support webhook verification when the selected provider offers it.
- [ ] Normalize vendor output into a provider-neutral result and record latency/cost.
- [ ] Add mocked contract tests and an opt-in live-provider test.
...
- [ ] Implement a provider-neutral commercial drum-transcription HTTP adapter.
- [ ] Validate and normalize simultaneous multi-label hits, velocity and calibrated confidence into raw canonical hits.
- [ ] Record provider request/model/cost/error metadata.
- [ ] Add mocked contract tests and an opt-in rights-cleared live test.
- [ ] If no real drum-specific commercial API is available, retain the fail-closed adapter contract and finish the self-hosted training path without pretending it is deployable.

### P0.4 Beat, bar and timing

- [ ] Implement canonical tempo segments, beat/downbeat records and bar-one offset persistence.
- [ ] Implement a production beat/downbeat adapter and opt-in live test.
- [ ] Add API operations for set bar one, BPM/time signature changes, beat insert/delete/move, tempo changes, reset and requantization.
- [ ] Preserve every raw drum onset while recomputing musical mapping.
- [ ] Support 4/4, 3/4, 6/8 and 12/8 extremely well; retain arbitrary validated signatures.
- [ ] Implement selective requantization with deliberate manual-edit preservation rules.
...
- [ ] Add 25/50/100 ms onset metrics, separation metrics/listening rubric and provider-combination matrix.
- [ ] Instrument correction burden: add/delete/move/reassign, tempo/bar corrections, correction minutes and corrections per audio minute.
- [ ] Report quality, latency, processing seconds/audio minute, estimated cost/audio minute and cost/successful transcription.
- [ ] Establish a measured baseline; never invent an accuracy target.

### P1.2 ML lifecycle

- [ ] Add configurable raw → validated → canonicalized → split → augmented → feature-cache stages.
- [ ] Implement deterministic, realistic augmentation with manifest provenance.
- [ ] Add configuration-driven training, resume/checkpoint, early stopping and metric logging.
- [ ] Record experiment ID, Git commit, dataset version/hash, configuration and model hash.
- [ ] Enforce licensing/attribution gates before any dataset or checkpoint becomes production eligible.

### P1.3 Product/editor architecture

- [ ] Add explicit Edit, Timing, Review and Practice modes.
- [ ] Make notation readable, paginated/chunked and primary; preserve event selection and current-measure highlighting.
- [ ] Build the Timing timeline and actions from P0.4.
...
- [ ] Add useful error/not-found states and remove broad demo fallback on API 404s.
...
- [ ] `DATA_RETENTION.md`
- [ ] `LAUNCH_CHECKLIST.md`
- [ ] Update README, architecture, licensing, evaluation, deployment, security and testing documents.

## Local and hosted release gates

- [ ] Provide `./scripts/ci.sh` and `make ci` parity for formatting, lint, typing, unit/integration tests, build, migrations and dependency audits.
- [ ] Keep GitHub workflows aligned with the local command.
- [ ] Resolve the owner account’s GitHub billing/spending-limit block, then rerun hosted CI.
- [ ] Require opt-in `RUN_LIVE_ML_TESTS=1` for credentialed provider tests.
