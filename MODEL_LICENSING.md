...
|---|---|---|---|---|---|---|---|
| Demucs v4 / `htdemucs_ft` and `htdemucs` | [facebookresearch/demucs](https://github.com/facebookresearch/demucs) (archived) | MIT for repository code | The four `htdemucs_ft` weights, bag hash, and single `htdemucs` checkpoint are pinned; the project's model-license issue distinguishes pretrained weights from MIT code and describes them as scientific-use material | Founder reports a separate commercial-inference grant, reaffirmed for the exact `htdemucs` checkpoint on 2026-09-13; source evidence is not archived here | **Conditionally asserted by founder; not independently verified** | Preserve MIT attribution; do not represent the private grant as part of the public upstream license | Application-approved separators; paid launch requires the actual separate grant covering the deployed exact weight |
...
operation on 2026-09-05, and reaffirmed the same asserted permission for the
exact `htdemucs` checkpoint on 2026-09-13. Exact source/checkpoint hashes and the asserted-use
